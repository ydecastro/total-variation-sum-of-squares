...
if ~exist('Xp')
 ...
 newpoints = input('choose new points ? (0/1)=');
end
if newpoints
 Xp = []; Xm = [];
 for i = 1:3 % random points on the sphere
...
% build problem data
d = 7; % degree
p = genpow(3,d); % homogeneous monomial powers
b = zeros(size(p,1),1);
...
try
 mext(Mp,3,2*k) % positive atoms
 mext(Mm,3,2*k) % negative atoms
end
% display polynomial certificate on the sphere
close all
[X1,X2,X3]=sphere(500);
certif=x(1:size(p,1))'*mmon(xp,d,d);
X4=eval(vectorize(certif,'X1','X2','X3'));
h=surf(X1,X2,X3,X4,'EdgeColor','none');
axis vis3d
camlight left
xlabel x_1
ylabel x_2
